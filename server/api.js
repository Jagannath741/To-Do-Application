...
app.get('/get-appointment/:id',(req,res)=>{
    mongoClient.connect(conString).then(clientObj=>{
        var database = clientObj.db("todo");
        database.collection("appointments").findOne({AppointmentId:parseInt(req.params.id)}).then(documents=>{
            res.send(documents);
            res.end();
        });
    });
...
app.delete('/delete-appointment/:id' ,(req, res)=>{
    ...
    mongoClient.connect(conString).then(clientObj=>{
        var database = clientObj.db("todo");
        database.collection('appointments').deleteOne({AppointmentId:parseInt(req.params.id)},{$set:appointment}).then(()=>{
